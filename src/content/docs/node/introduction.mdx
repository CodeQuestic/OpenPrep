...
import snippet2 from "./codes/introductionCode/eventloop.txt?raw";
...
## 1. Basic Installation Steps for Node.js
...
## Creating Your First Node.js Application: A Step-by-Step Tutorial
...
<Code code={snippet} lang="js" title="app.js" />
...
4.  **Callbacks**: When an asynchronous operation completes, a callback is queued to be executed. The event loop picks up these callbacks and executes them one by one, ensuring that the program doesn't block while waiting for long-running tasks.

    **Example**

    <Code code={snippet2} lang="js" title="js" />
...
CommonJS uses `module.exports` and `require` for exporting and importing modules, while ES Modules use `export` and `import`.
